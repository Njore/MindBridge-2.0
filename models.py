...
class Capsule(db.Model):
    ...
    # Read/unread tracking per viewer
    # therapist_read: True once the therapist has opened the capsule; reset to False when client sends a new message
    # client_read: True once the client has opened the capsule; reset to False when therapist replies
    therapist_read = db.Column(db.Boolean, default=False, nullable=False)
    client_read = db.Column(db.Boolean, default=False, nullable=False)
...
class CrisisEvent(db.Model):
    ...
    relationship_id = db.Column(db.Integer,
                                db.ForeignKey('client_therapist_relationships.relationship_id', ondelete='CASCADE'),
                                nullable=False)
    crisis_type = db.Column(
        db.Enum('suicidal_ideation', 'severe_panic', 'severe_anxiety', 'self_harm_urge', 'dissociation', 'other'),
        nullable=False)
...
class DeescalationTechnique(db.Model):
    ...
    technique_category = db.Column(
        db.Enum('grounding', 'breathing', 'mindfulness', 'distraction', 'physical', 'emotional_regulation',
                'thought_challenging'), nullable=False)
    description = db.Column(db.Text, nullable=False)
...
class UserPrivacySetting(db.Model):
    ...
    share_progress_with_therapist = db.Column(db.Boolean, default=False)
...
class PasswordResetRequest(db.Model):
    ...
    status = db.Column(db.Enum('pending', 'approved', 'used', 'rejected'), default='pending', nullable=False)
    requested_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
